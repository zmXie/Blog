...
- Promise 是对异步操作的封装，有三种状态：`pending`(进行中)、`fulfilled`(已完成)、`rejected`(已失败)，默认为`pending`状态，当异步操作完成时，状态会变为`fulfilled`或者`rejected`，接着会调用通过`then`方法注册的回调函数来处理异步结果，这样就可以将异步操作以同步的方式表达出来。
